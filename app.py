...
from unetutils import (
    create_and_load_unet_model,
    test_single_image_streamlit,
    process_video_streamlit_old  # dùng hàm xử lý video cũ
)
...
if uploaded_file is not None:
    ...
    st.markdown(f"**🗂️ Tệp đã chọn:** {uploaded_file.name} – {uploaded_file.size / 1024:.1f} KB")
    ...
    if uploaded_file.type.startswith('image'):
        ...
        with col1:
            st.video(file_path)
            st.markdown("<div style='text-align:center'>🎥 Video gốc</div>", unsafe_allow_html=True)

        output_path = None
        if st.button("▶️ Bắt đầu xử lý video"):
            with st.spinner("⚙️ Đang xử lý video..."):
                output_path = process_video_streamlit_old(file_path, model, transform_img, device)
            st.success("✅ Video đã được xử lý xong!")

        if output_path:
            with col2:
                st.video(output_path)
                st.markdown("<div style='text-align:center'>🎯 Video kết quả</div>", unsafe_allow_html=True)

# ==== FOOTER ==== #
st.markdown("<hr style='margin-top:3rem;'>", unsafe_allow_html=True)
st.markdown("<div style='text-align:center'>Developed by <b>Trần Quý Thế – 20THXD1</b></div>", unsafe_allow_html=True)
